...
This is a tutorial for running [CENTIPEDE] with [DNase-Seq] data. By following
this tutorial, you will learn how to determine if a putative transcription
factor binding site is actually bound or not. At the end, you will see how
to incorporate other information such as sequence conservation across
vertebrates.
...
species. The method was created by [Roger Pique-Regi] when he was working in
[Jonathan Pritchard]'s group at University of Chicago in 2011.
...
Start an R session and create a matrix with read start counts. Each row in
the matrix represents a site where the PWM matches the sequence.

```{r}
library(CENTIPEDE.tutorial)
```

I've already included the result from the call to "centipede_data", so you can
skip this line if you wish to save time.

```{r, eval=FALSE}
cen <- centipede_data('ENCFF000SHS.bam', 'M6496_1.02.fimo.txt.gz')
```
...
Compute mean conservation scores for each PWM site. I've already included the
result of this chunk of code, so you can skip it if you wish to save time.
